...
//        mailService.sendHTMLMail(dto);
    ...
    //会员注册
    public Integer register_vip(Gms_Vip gms_vip){
        Integer res = userMapper.register_vip(gms_vip);
        return res;
    }
    //测试会员是否已经存在(通过身份证号)
    public Gms_Vip finVipById(String id){
        return userMapper.finVipById(id);
    }

    //测试会员是否已经存在
    public Gms_Vip finVipByPhone(String phone){
        return userMapper.finVipByPhone(phone);
    }
    ...
    public void memberpay(String vip_Phone,String vip_NewAccount){
        userMapper.memberpay(vip_Phone,vip_NewAccount);
    }

    public String getAccount(){
        String vip_Phone = SecurityContextHolder.getContext().getAuthentication().getName();
        String vip_Account = userMapper.finVipAccountById(vip_Phone);
        return vip_Account;
    }

    //测试会员是否已经存在(通过手机号user_Id)
    public Gms_Vip finVipByUserid(String id){
        return userMapper.finVipByUserid(id);
    }

    //获取学号
    public String getVipUserid(){
        String vip_Phone = SecurityContextHolder.getContext().getAuthentication().getName();
        String vip_Userid = userMapper.finVipUseridByPhone(vip_Phone);
        return vip_Userid;
    }

    public String getVipId(){
        String vip_Phone = SecurityContextHolder.getContext().getAuthentication().getName();
        String vip_Id = userMapper.finVipIdByPhone(vip_Phone);
        return vip_Id;
    }
}
