...
public class UserController {
    ...
    //会员注册部分
    @PostMapping("/register_vip")
    public Map<String,String> register_vip(@RequestBody Gms_Vip gms_vip){
        Map<String,String> map = new HashMap<>();
        Integer res = userService.register_vip(gms_vip);
        if(res > 0){
            map.put("state","1");
            map.put("info","会员注册成功");
        }else {
    ...
    @GetMapping("/checkVipname")
    public Map<String,Object> checkVipname(@RequestParam Map<String,String> resmap){
        Map<String,Object> map = new HashMap<>();
        Gms_Vip vip = userService.finVipById(resmap.get("vip_Id"));
        if(vip != null){
            map.put("info",vip);
        }else {
            map.put("info",null);
    ...
    @GetMapping("/checkVipPhone")
    public Map<String,Object> checkVipPhone(@RequestParam Map<String,String> resmap){
        Map<String,Object> map = new HashMap<>();
        Gms_Vip vip = userService.finVipByPhone(resmap.get("vip_Phone"));
        if(vip != null){
            map.put("info",vip);
        }else {
            map.put("info",null);
    ...
    public String memberpay(@RequestBody Map<String, String>  map){
        String vip_Phone = map.get("vip_Phone");
        String vip_NewAccount = map.get("vip_NewAccount");
        userService.memberpay(vip_Phone,vip_NewAccount);
        return "{}";
    }
    ...
    public Map<String,Object> checkmember(@RequestParam Map<String,String> resmap){
        Map<String,Object> map = new HashMap<>();
        Gms_Vip vip = userService.finVipByUserid(resmap.get("user_Id"));
        if(vip != null){
            map.put("info",1);
        }else {
            map.put("info",null);
    ...
    @GetMapping("/getVipUserid")
    public String getVipUserid(){
        return userService.getVipUserid();
    }

    @GetMapping("/getVipId")
    public String getVipId(){
        return userService.getVipId();
    }
